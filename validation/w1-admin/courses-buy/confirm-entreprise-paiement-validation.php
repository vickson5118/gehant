...
$beneficiaireEmail = Functions::getValueChamp($_POST["email"]);
$beneficiaireName = Functions::getValueChamp($_POST["name"]);
$souscripteurName = Functions::getValueChamp($_POST["entreprise"]);
...
if($achatManager -> updateConfirmParticulierPaid($factureId, intval($achatId))){
    ...
    try{
       ...
       $altMessage = "Bonjour " . $souscripteurName. "\n\nVotre inscription par l'entreprise ".$souscripteurName." à la formation "
           .$formationTitre." qui se tiendra du ".$formationDate." à ".$formationLieu." a bien été pris en compte..\n\n
                    L'equipe GEHANT vous contactera sous peu pour la l'organisation de la formation.\n\nA bientôt,\nL'équipe GEHANT";
        
                try {
                    
                    //envoyer le mail
                    if(Functions::sendMail($beneficiaireEmail, $objet, $message, $altMessage)){  
                        echo json_encode(array(
                            "type" => "success"
                        ));
