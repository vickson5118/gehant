...
foreach($listeFormation as $formation){
    ...
    if( !$formation -> getBloquer() && $formation -> getRedactionFinished()){
        $formationActif = true;
    }

    if( !$formation -> getRedactionFinished()){
        $formationWriting = true;
    }

    if($formation -> getBloquer()){
...
<body>
					...
					<table class="table table-hover table-bordered">
						...
						<tbody>
							<?php
                                ...
                                if( !$courseWriting -> getRedactionFinished()){
                                    $indexCountWriting++;
                               ?>
        							<tr>
								...
								<td><a href="/w1-admin/creation/formation/<?= $courseWriting->getDomaine()->getTitreUrl() ?>/<?= $courseWriting->getTitreUrl() ?>/prerequis"
    ...
    $indexCountOnLine = 0;
        foreach($listeFormation as $courseActif){
            if( !$courseActif -> getBloquer() && $courseActif -> getRedactionFinished() && Functions::convertDateFrToEn($courseActif ->getDateDebut()) >= date("Y-m-d")){
                $indexCountOnLine++;
                ?>
							<tr>
								...
								<td>
    								<a href="/w1-admin/liste-acheteur/<?= $courseActif->getId() ?>" class="btn btn-primary" title="Voir les achéteurs de la formation"> <i class="bi bi-person-fill"></i></a>
    								<a href="/w1-admin/apercu/formation/<?= $courseActif->getDomaine()->getTitreUrl() ?>/<?= $courseActif->getTitreUrl() ?>" class="btn btn-success" title="Aperçu de la formation"> <i class="bi bi-eye-fill"></i></a>
    								<a href="/w1-admin/creation/formation/<?= $courseActif->getDomaine()->getTitreUrl() ?>/<?= $courseActif->getTitreUrl() ?>/prerequis" class="btn btn-primary" title="Modifier la formation"> <i class="bi bi-pencil-fill"></i></a>
    								<button data-bs-toggle="modal" data-bs-target="#modalSendMail" type="button" class="btn btn-primary btn-send-mail" title="Envoyer un mail à l'auteur"><i class="bi bi-envelope"></i></button>
    								<button data-bs-toggle="modal" data-bs-target="#staticBackdropBloquerFormation" value="<?= $courseActif->getId() ?>" type="button" class="btn btn-danger btn-bloquer-formation" title="Bloquer la formation"><i class="bi bi-lock-fill"></i></button>
								</td>
							</tr>
							<?php }}?>
						</tbody>
            ...
            if(Functions::convertDateFrToEn($courseExpired ->getDateDebut()) < date("Y-m-d")){
                ...
                ?>
							<tr>
								...
								<td>
									<a href="/w1-admin/apercu/formation/<?= $courseExpired->getDomaine()->getTitreUrl() ?>/<?= $courseExpired->getTitreUrl() ?>" class="btn btn-success" title="Aperçu de la formation"> <i class="bi bi-eye-fill"></i></a>
    								<a href="/w1-admin/liste-acheteur/<?= $courseExpired->getId() ?>" class="btn btn-primary" title="Voir les achéteurs de la formation"> <i class="bi bi-person-fill"></i></a>
    								<a href="/w1-admin/creation/formation/<?= $courseExpired->getDomaine()->getTitreUrl() ?>/<?= $courseExpired->getTitreUrl() ?>/prerequis" class="btn btn-primary" title="Modifier la formation"> <i class="bi bi-pencil-fill"></i></a>
    								<button data-bs-toggle="modal" data-bs-target="#modalSendMail" type="button" class="btn btn-primary btn-send-mail" title="Envoyer un mail à l'auteur"><i class="bi bi-envelope"></i></button>
								</td>
							</tr>
							<?php }}?>
        							...
        							<tr>
        								...
        								<td>
        									...
        									<button data-bs-toggle="modal"
        										data-bs-target="#staticBackdropDebloquerFormation" value="<?= $courseLocked->getId() ?>"
