							...
							<div>
								...
								<form id="objectifs-form">
									<?php if($formation->getObjectifs() == null){ ?>
										<input type="text" id="objectifs-1" class="form-control" placeholder="Exemple : Reussir à créer une application mobile multiplateforme">
    									<?php }else{
    										    $listeObjectif = explode(";",$formation->getObjectifs());
    										    foreach ($listeObjectif as $key => $objectif){
    								    ?>
    										<input type="text" value="<?= $objectif ?>" id="objectifs-<?= ($key+1) ?>" class="form-control" placeholder="Exemple : Reussir à créer une application mobile multiplateforme">
    									<?php } ?>
									<?php } ?>	
								</form>
