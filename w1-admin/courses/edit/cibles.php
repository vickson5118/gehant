			...
			<div class="row">
				...
				<div class="col-md-9 formation-infos-container">
					...
					<div class="section-content">
						<p>Les prérequis suivants seront affichés sur le page d'accueil du cours. Ces prérequis doivent respecter les spécifications suivanes:</p>
						...
						<div>
							<div>
								...
								<p>Rédigez une courte description des participants cibles que le contenu de votre cours peut intéresser. Cela vous aidera à attirer les bons participants.</p>
								<form id="cibles-form">
									<?php if($formation->getPrerequis() == null){ ?>
										<input type="text" id="cibles-1" class="form-control" placeholder="Exemple : Developpeur Web">
    									<?php }else{
    										    $listeCible = explode(";",$formation->getCibles());
    										    foreach ($listeCible as $key => $cible){
    									   ?>
    										<input type="text" value="<?= $cible?>" id="cibles-<?= ($key+1) ?>" class="form-control" placeholder="Exemple : Developpeur Web">
    									<?php } ?>
									<?php } ?>
								</form>
