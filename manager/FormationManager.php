...
class FormationManager
{
    ...
    public function addFormation(Formation $formation): bool
    {
        try {
            ...
            $prepare = Functions::bindPrepare($bdd, $sql, $formation->getTitre(), $formation->getTitreUrl(), $formation->getDateCreation(), $formation->getDomaine()->getId(), $formation->getAuteur()->getId());
        ...
        try {
            ...
            if ($isAdmin) {
                $sql = "SELECT f.id,f.titre,f.titre_url,f.but,f.objectifs,f.prerequis,f.cibles,f.description,f.prix,f.illustration,
                        ...
                        d.titre AS domaine_titre,d.titre_url AS domaine_titre_url FROM " . Constants::TABLE_FORMATION . " AS f INNER JOIN " . Constants::TABLE_UTILISATEUR . " AS u INNER JOIN " . Constants::TABLE_DOMAINE . " AS d ON f.auteur_id=u.id AND f.domaine_id = d.id WHERE d.titre_url=? AND f.titre_url=?";
            } else {
                $sql = "SELECT f.id,f.titre,f.titre_url,f.but,f.objectifs,f.prerequis,f.cibles,f.description,f.prix,f.illustration,
                        f.date_debut,f.date_fin,f.lieu,f.redaction_finished,f.bloquer,u.id AS auteurId,u.prenoms,u.nom,d.id AS domaine_id,
                        d.titre AS domaine_titre,d.titre_url AS domaine_titre_url FROM " . Constants::TABLE_FORMATION . " AS f INNER JOIN " . Constants::TABLE_UTILISATEUR . " AS u INNER JOIN " . Constants::TABLE_DOMAINE . " AS d ON f.auteur_id=u.id AND f.domaine_id = d.id WHERE d.titre_url=? AND f.titre_url=? AND f.bloquer=false 
    ...
    public function updateFormationPrerequis(Formation $formation): bool
    {
        try {
            ...
            $prepare = Functions::bindPrepare($bdd, $sql, $formation->getPrerequis(), $formation->getId());
    ...
    public function updateFormationCibles(Formation $formation): bool
    {
        try {
            ...
            $prepare = Functions::bindPrepare($bdd, $sql, $formation->getCibles(), $formation->getId());
    ...
    public function updateFormationObjectifs(Formation $formation): bool
    {
        try {
            ...
            $prepare = Functions::bindPrepare($bdd, $sql, $formation->getObjectifs(), $formation->getId());
    ...
    public function updateBasicsWithImageFile(Formation $formation): bool
    {
        try {
            ...
            $prepare = Functions::bindPrepare($bdd, $sql, $formation->getTitre(), $formation->getTitreUrl(), $formation->getBut(), $formation->getDomaine()->getId(), $formation->getPrix(), $formation->getDescription(), $formation->getIllustration(), $formation->getLieu(), $formation->getDateDebut(), $formation->getDateFin(), $formation->getId());
    ...
    public function updateBasicsWithoutImageFile(Formation $formation): bool
    {
        try {
            ...
            $prepare = Functions::bindPrepare($bdd, $sql, $formation->getTitre(), $formation->getTitreUrl(), $formation->getBut(), $formation->getDomaine()->getId(), $formation->getPrix(), $formation->getDescription(), $formation->getLieu(), $formation->getDateDebut(), $formation->getDateFin(), $formation->getId());
    ...
    public function getFourLastFormation(): array
    {
        try {
            $bdd = Database::getInstance();
            $sql = "SELECT f.titre,f.titre_url,f.illustration,d.titre_url AS domaine_titre_url FROM " . Constants::TABLE_FORMATION . " AS f INNER JOIN " . Constants::TABLE_DOMAINE . " AS d ON f.domaine_id=d.id WHERE f.date_debut>NOW() AND
    ...
    public function getTwelveLastFormationWithoutThis(?int $id): array
    {
        try {
            ...
            $sql = "SELECT f.titre,f.titre_url,f.illustration,f.date_debut,f.date_fin,d.titre_url AS domaine_titre_url,u.nom,u.prenoms 
                FROM " . Constants::TABLE_FORMATION . " AS f INNER JOIN " . Constants::TABLE_DOMAINE . " AS d INNER JOIN " . Constants::TABLE_UTILISATEUR . " AS u ON f.domaine_id=d.id AND f.auteur_id = u.id WHERE f.id!=? AND 
    ...
    public function getTwelveLastFormation(): array
    {
        try {
            ...
            $bdd = Database::getInstance();
            $sql = "SELECT f.titre,f.titre_url,f.illustration,f.date_debut,f.date_fin,d.titre_url AS domaine_titre_url,u.nom,u.prenoms
                FROM " . Constants::TABLE_FORMATION . " AS f INNER JOIN " . Constants::TABLE_DOMAINE . " AS d INNER JOIN " . Constants::TABLE_UTILISATEUR . " AS u ON f.domaine_id=d.id AND f.auteur_id = u.id WHERE f.date_debut>NOW() 
    ...
    public function getOneFormationInfoById(int $id): ?Formation
    {
        try {
            $bdd = Database::getInstance();
            $sql = "SELECT f.id,f.titre,f.titre_url,f.prix,f.date_debut,f.date_fin,f.lieu,u.prenoms,u.nom,d.titre_url AS domaineUrl FROM " . Constants::TABLE_FORMATION . " AS f INNER JOIN " . Constants::TABLE_UTILISATEUR . " AS u ON f.auteur_id=u.id 
    ...
    public function searchFormation(string $search): array
    {
        try {
            $bdd = Database::getInstance();
            $sql = "SELECT f.titre,f.titre_url,d.titre_url AS domaine_url FROM " . Constants::TABLE_FORMATION . " AS f INNER JOIN " . Constants::TABLE_DOMAINE . " AS d ON f.domaine_id=d.id WHERE MATCH(f.titre) AGAINST('" . $search . "*" . "' IN BOOLEAN MODE) AND f.redaction_finished=true AND f.bloquer=false";
            ...
            while ($result = $prepare->fetch()) {
                ...
                $formation = array("titre" => $result["titre"], "titreUrl" => $result["titre_url"], "domaineUrl" => $result["domaine_url"],);
     ...
     */
    public function updateNombreAchat(string $formationTitre): void
    {
        try {
            $bdd = Database::getInstance();
